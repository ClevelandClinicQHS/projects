...
```{r setup_projects}
library(projects)
setup_projects("/home/kriegen/Projects/projects/test")
```
...
new_author(given_names = "Scott", last_name = "Bug", title = "Professor",
           affiliations = c(2, "Physics"), degree = "Ph.D.",
           email = "scottbug@impossible.net", phone = "965-555-5556",
           default = TRUE)
```

Notice that in creating associations between Scott Bug and his affiliations, we were able to enter both the `id` number of one (2) and a substring of the `department_name` of the other ("Physics").
