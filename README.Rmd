...
library(fs)
library(tidyverse)
if(fs::dir_exists("C:/projects")) fs::dir_delete("C:/projects")
```
...
```{r affiliation2, echo = TRUE, results = 'hide', message=FALSE}
...
```{r authors2, echo = TRUE, results = 'hide', message=FALSE}
...
```{r print_header, echo = FALSE, comment = NA}
fs::path(projects("Cold Fusion")$path, "progs/01_protocol.Rmd") %>% 
  readr::read_lines() %>% 
  `[`(1:grep("\\pagebreak", x = .)[1]) %>% 
  cat(sep = "\n")
```
...
```{r new_project2, echo = TRUE, results = 'hide', message = FALSE}
...
```{r edit_affil, echo = TRUE, results = 'hide', message=FALSE}
...
```{r move_proj, echo=TRUE, results='hide', message=FALSE}
